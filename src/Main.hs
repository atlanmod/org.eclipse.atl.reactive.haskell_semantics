...
newtype Config' = Config' { unConfig'::([Instruction],Model,Transformation) } deriving (Show,Eq)
...
configs' = do
  ...
  -- a configuration
  return $ Config' (instructions,mStripped,(rootS,rootT):transfo)

instance Arbitrary Config' where
    arbitrary = configs'
                           
test' = verboseCheckWith stdArgs { maxSuccess = 1000 } assertion'
